...
// The only difference is that we are using a different consumer and nothing else
func attemptTwelve(workers, chanBufSize int) {
	inputChannels := make([]chan []byte, workers)
	outputChannels := make([]chan map[uint64]*StationData, workers)
	...
	for i := range workers {
		...
		output := make(chan map[uint64]*StationData, 1)

		go consumerV4(input, output, &wg)
	...
	data := make(map[uint64]*StationData)
	for i := range workers {
